(define (domain Rover)
  ...
  (:types
      time_constraint_entity  ;; New base type for entities with a time constraint
      rover waypoint - time_constraint_entity store camera mode lander objective - time_constraint_entity  ;; waypoint and objective now inherit from time_constraint_entity
  )
  ...
  (:predicates
      ...
      (in_sun ?w - waypoint)
      (still-on-time ?entity - time_constraint_entity)  ;; updated predicate: it's still 'on time' to achieve the goal for the entity (either waypoint or objective)
  )
  ...
  ;; Durative actions here (navigate, recharge, sample_soil, etc.) should be updated to reflect the new 'still-on-time' predicate's usage.
  ;; Below is an example of how you might adjust the 'navigate' action:
  ...
  (:durative-action navigate
      ...
      :condition
          (and
              ...
              (over all (still-on-time ?z))  ;; ensuring the destination waypoint is still relevant, not just objectives
          )
      :effect
          (and
...
(:durative-action recharge
    ...
    :condition
        (and
            ...
            (over all (still-on-time ?w))  ;; added condition for timeliness
        )
    :effect
        (and
...
(:durative-action sample_soil
    ...
    :condition
        (and
            ...
            (at start (empty ?s))
            (over all (still-on-time ?p))  ;; added condition: the soil sampling must be still on time
        )
    :effect
        (and
...
(:durative-action sample_rock
    ...
    :condition
        (and
            ...
            (at start (store_of ?s ?x))
            (at start (empty ?s))
            (over all (still-on-time ?p))  ;; added condition for timeliness
        )
    :effect
        (and
            (at start (not (empty ?s)))
...
(:durative-action calibrate
    ...
    :condition
        (and
            ...
            (over all (still-on-time ?t))  ;; added condition for timeliness
        )
    :effect
        (and
...
(:durative-action take_image
    ...
    :condition
        (and
            ...
            (over all (at ?r ?p))
            (at start (>= (energy ?r) 1))
            (over all (still-on-time ?o))  ;; added condition: it's still 'on time' for the imaging objective
        )
    :effect
        (and
...
(:durative-action communicate_image_data
    ...
    :condition
        (and
            ...
            (at start (channel_free ?l))
            (at start (>= (energy ?r) 6))
            (over all (still-on-time ?o))  ;; added condition for timeliness
        )
    :effect
        (and
            (at start (not (available ?r)))
